...
export const OrdenDeCompraRegistrosAdmin = () => {
  ...
  const obtenerOrdenesRangoFechas = async (fechaInicio, fechaFin) => {
    try {
      ...
      const response = await client.post("/ordenes-rango-fechas-admin", {
        fechaInicio,
        fechaFin,
      });
  ...
  // Obtener todas las localidades únicas para el control de selección
  const localidades = Array.from(
    new Set(ordenesBuscadas.map((orden) => orden.localidad_usuario))
  );

  // Filtrar productos basados en términos de búsqueda, categoría seleccionada y localidad seleccionada
  const filteredProducts = ordenesBuscadas.filter((orden) => {
    ...
    const matchesLocalidad =
      ...
      orden.localidad_usuario === selectedLocalidad;

    return matchesSearchTerm && matchesCategory && matchesLocalidad;
  });
            ...
            {currentProducts.map((o) => (
              ...
              >
                <div className="absolute top-2 right-5 flex items-center gap-2">
                  <Link
                    to={`/orden/${o.id}`}
                    className="py-2 px-4 rounded-xl text-sky-600 font-semibold text-xs bg-sky-100 cursor-pointer flex items-center gap-1"
                  >
                    VER ORDEN
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                ...
                <article>
                  <p className="uppercase flex gap-1">
                    <span className="font-bold text-sm text-gray-800 underline">
                      Numero
                    </span>
