...
export const ProductosOrdenesFiltradorAdmin = () => {
  ...
  // Filtrar productos según búsqueda, categoría, y localidad
  const filteredProducts = ordenesBuscadas.flatMap((orden) =>
    orden.datos.productoSeleccionado
      .map((producto) => ({
        ...producto,
        proveedor: orden.proveedor,
        localidad: orden.localidad_usuario, // Se agrega la localidad del usuario a cada producto
      }))
      .filter(
        (producto) =>
          (producto.detalle.toLowerCase().includes(searchTerm.toLowerCase()) ||
            producto.proveedor
              .toLowerCase()
              .includes(searchTerm.toLowerCase())) && // Búsqueda por detalle o proveedor
          (selectedCategory === "all" ||
            producto.categoria === selectedCategory) && // Filtro por categoría
          (selectedLocalidad === "all" ||
            producto.localidad === selectedLocalidad) // Filtro por localidad
      )
  );
